# -*- coding: utf-8 -*-
import os, sys, jmp, numpy as np, pandas as pd
...
class OptProblem(ElementwiseProblem):
    ...
    def _evaluate(self, x:np.ndarray, out:dict, *args, **kwargs):
        dtFormula["StreamTime"][0] = round(x[0])
        dtFormula["CategoryCode"][0] = round(x[1])

        if dtFormula["CategoryCode"][0] > category_codes[-1]:
            dtFormula["CategoryCode"][0] = category_codes[-1]

        jmp.run_jsl("dtFormula << RerunFormulas;")

        out["F"] = [
            dtFormula["Viewers Predictor"][0] * -1
        ]

        # history_datas.append({
        #     "Step": len(history_datas) + 1,
        #     "StreamTime": dtFormula["StreamTime"][0],
        #     "CategoryCode": dtFormula["CategoryCode"][0],
        #     "Viewers": out["F"][0] * -1
        # })


# run script
jmp.run_jsl(f"dtFormula << {formulaText};")
